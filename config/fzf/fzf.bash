# Load fzf key-bindings + completion for bash.
# Stops after the first hit so apt+brew double-installs don't double-bind.

for _fzf_kb in \
    /usr/share/doc/fzf/examples/key-bindings.bash \
    /usr/share/fzf/key-bindings.bash \
    "$HOME/.fzf/shell/key-bindings.bash"; do
    if [ -r "$_fzf_kb" ]; then
        source "$_fzf_kb"
        break
    fi
done

for _fzf_cmp in \
    /usr/share/doc/fzf/examples/completion.bash \
    /usr/share/fzf/completion.bash \
    "$HOME/.fzf/shell/completion.bash"; do
    if [ -r "$_fzf_cmp" ]; then
        source "$_fzf_cmp"
        break
    fi
done

if [ -z "$_fzf_kb" ] && command -v brew >/dev/null 2>&1; then
    _fzf_prefix=$(brew --prefix 2>/dev/null)
    [ -r "$_fzf_prefix/opt/fzf/shell/key-bindings.bash" ] && source "$_fzf_prefix/opt/fzf/shell/key-bindings.bash"
    [ -r "$_fzf_prefix/opt/fzf/shell/completion.bash" ] && source "$_fzf_prefix/opt/fzf/shell/completion.bash"
    unset _fzf_prefix
fi
unset _fzf_kb _fzf_cmp
